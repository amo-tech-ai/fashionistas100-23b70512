...
async function testAllDashboards() {
  ...
  const tables = [
    'events', 'bookings', 'venue_bookings', 'designer_profiles',
    'model_profiles', 'venues', 'event_registrations', 'attendees'
  ];
  ...
  for (const table of tables) {
    ...
    if (!error) {
      ...
      results.failed.push(`Table '${table}' not accessible`);
    }
  }
  ...
  console.log('\n📝 RECOMMENDATIONS:');
  if (totalRevenue === 0) {
    console.log('  1. Add sample revenue data to bookings/venue_bookings');
  }
  if ((registrations || 0) + (attendees || 0) === 0) {
    console.log('  2. Add test registrations/attendees');
  }
  if (models === 0) {
    console.log('  3. Create model profiles');
  }
  if (designers === 0) {
    console.log('  4. Create designer profiles');
  }
}
