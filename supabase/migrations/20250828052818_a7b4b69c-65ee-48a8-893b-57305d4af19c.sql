-- Final attempt: adjust schema minimal, then insert dataset (omit generated columns)

-- 1) Allow null organizer_id to load public sample data
...
-- 4) Tickets (omit generated available_quantity)
...
((SELECT id FROM events WHERE event_name = 'Medellín Fashion Week Otoño 2025'), 'General Access', 'basic', 'Acceso a todos los desfiles, app del evento, welcome drink y programa digital', 140.00, 'USD', 800, 120, 'active'),
((SELECT id FROM events WHERE event_name = 'Medellín Fashion Week Otoño 2025'), 'VIP Experience', 'premium', 'Asientos preferenciales, encuentro con diseñadores, acceso a lounge VIP, bolsa de regalo y after-party', 280.00, 'USD', 200, 45, 'active'),
((SELECT id FROM events WHERE event_name = 'Medellín Fashion Week Otoño 2025'), 'Platinum Elite', 'vip', 'Primera fila, acceso backstage, styling personalizada, concierge, NFT exclusiva y todas las experiencias VIP', 500.00, 'USD', 50, 8, 'active'),

-- Alta Costura Gala tickets
((SELECT id FROM events WHERE event_name = 'Noches de Alta Costura Paisa'), 'Premium Gala', 'premium', 'Cena de 4 tiempos, cóctel de bienvenida, meet & greet con diseñadores y bolsa de regalo de lujo', 220.00, 'USD', 250, 89, 'active'),
((SELECT id FROM events WHERE event_name = 'Noches de Alta Costura Paisa'), 'Diamond Experience', 'vip', 'Todas las experiencias premium más acceso exclusivo a after-party y consulta de styling', 400.00, 'USD', 50, 12, 'active'),

-- Sustainable Fashion Summit tickets  
((SELECT id FROM events WHERE event_name = 'Cumbre de Moda Sostenible Medellín'), 'Professional Pass', 'basic', 'Acceso completo a conferencias, materiales digitales y coffee breaks', 85.00, 'USD', 350, 76, 'active'),
((SELECT id FROM events WHERE event_name = 'Cumbre de Moda Sostenible Medellín'), 'Student Pass', 'basic', 'Acceso completo con descuento estudiantil', 45.00, 'USD', 100, 34, 'active'),
((SELECT id FROM events WHERE event_name = 'Cumbre de Moda Sostenible Medellín'), 'VIP Networking', 'premium', 'Acceso completo más almuerzo exclusivo, networking session y certificate', 150.00, 'USD', 50, 18, 'active'),

-- Urban Fashion Festival (free entry, but VIP packages available)
((SELECT id FROM events WHERE event_name = 'Festival de Moda Urbana El Poblado'), 'General Entry', 'basic', 'Entrada gratuita al festival con acceso a todos los stands y actividades', 0.00, 'USD', 2000, 234, 'active'),
((SELECT id FROM events WHERE event_name = 'Festival de Moda Urbana El Poblado'), 'VIP Package', 'premium', 'Área VIP, comida gourmet, meet & greet con artistas y acceso prioritario a talleres', 35.00, 'USD', 100, 23, 'active'),

-- Bridal Fashion Experience tickets
((SELECT id FROM events WHERE event_name = 'Medellín Bridal Fashion Experience'), 'General Access', 'basic', 'Acceso a desfiles nupciales, exhibición de joyería y degustación de postres', 90.00, 'USD', 200, 67, 'active'),
((SELECT id FROM events WHERE event_name = 'Medellín Bridal Fashion Experience'), 'VIP Bridal', 'premium', 'Consultas de styling personalizadas, sesión fotográfica profesional y acceso a experiencias exclusivas', 180.00, 'USD', 50, 15, 'active');
